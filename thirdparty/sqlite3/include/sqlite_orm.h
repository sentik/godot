...
namespace sqlite_orm {
    ...
    namespace internal {
        inline void perform_step(sqlite3* db, sqlite3_stmt* stmt) {
			...
			const auto sql_error_code = sqlite3_errcode(db);
			const auto &sql_error_category = get_sqlite_error_category();
			const auto sql_error_message = sqlite3_errmsg(db);
			const auto std_sql_error_code = std::error_code(sql_error_code, sql_error_category);

			throw std::system_error(std_sql_error_code, sql_error_message);
            }
        }

        static void perform_void_exec(sqlite3* db, const std::string& query) {
			const auto query_str = query.c_str();
			int rc = sqlite3_exec(db, query_str, nullptr, nullptr, nullptr);
            if(rc != SQLITE_OK) {
				const auto sql_error_code = sqlite3_errcode(db);
				const auto sql_extended_errcode = sqlite3_extended_errcode(db);
				const auto& sql_error_category = get_sqlite_error_category();
				const auto sql_error_message = sqlite3_errmsg(db);
				const auto std_sql_error_code = std::error_code(sql_error_code, sql_error_category);
				throw std::system_error(std_sql_error_code, sql_error_message);
...
namespace sqlite_orm {
    ...
    namespace internal {
        ...
        struct connection_holder {
          ...
          protected:
            sqlite3* db = nullptr;
            std::atomic<int> _retain_count = 0;
        };
        ...
        struct storage_t : storage_base {
          ...
          protected:
            ...
            prepared_statement_t<S> prepare_impl(S statement) {
				...
				const auto query_str = query.c_str();
				if (sqlite3_prepare_v2(db, query_str, -1, &stmt, nullptr) == SQLITE_OK) {
